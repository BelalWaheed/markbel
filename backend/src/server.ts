    ...
    if (shouldScrapeTitle || shouldScrapeImage || shouldScrapeDesc) {
      (async () => {
        try {
          ...
          if (!ytId || shouldScrapeTitle || shouldScrapeDesc) {
            try {
              ...
              if (response.ok) {
                ...
                scrapedTitle = getMetaTag('og:title') || getMetaTag('twitter:title') || getTitle()
                scrapedDesc = getMetaTag('og:description') || getMetaTag('twitter:description') || ''

                if (!ytId) {
                  scrapedImage = getMetaTag('og:image') || getMetaTag('twitter:image') || ''
          ...
          } else if (shouldScrapeTitle && ytId && (updatedTitle === url || updatedTitle === parsedUrl.hostname)) {
...
app.put('/api/bookmarks/group', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { oldName, newName } = req.body
    if (!oldName || !newName) {
      res.status(400).json({ error: 'Old group name and new group name are required' })
      return
    }
...
app.get('/api/bookmarks/meta', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    let title = ''
    let description = ''
    let image = ytId ? `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` : ''
    ...
    try {
      ...
      if (response.ok) {
        ...
        title = getMetaTag('og:title') || getMetaTag('twitter:title') || getTitle()
        description = getMetaTag('og:description') || getMetaTag('twitter:description') || ''
    ...
    const finalTitle = title ? decodeHtml(title.trim()) : (ytId ? 'YouTube Video' : parsedUrl.hostname)

...
import { bookmarkRepository, groupRepository } from "../db/SyncRepository.js";
...
export default function BookmarksPage() {
  ...
  const updateGroupColor = async (groupName: string, color: string) => {
    const group = dbGroups.find((g) => g.name === groupName);
    if (group) {
      await groupRepository.update(group.id, { color });
  ...
  const renameGroupColor = async (
    oldName: string,
    newName: string,
    color: string,
  ) => {
    const group = dbGroups.find((g) => g.name === oldName);
    if (group) {
      await groupRepository.update(group.id, { name: newName, color });
      loadBookmarks();
    }
  };
  ...
  const loadBookmarks = async () => {
    try {
      ...
      const [data, loadedGroups] = await Promise.all([
      ...
      setBookmarks(data);
      setDbGroups(loadedGroups);
      loadStats();
    } catch (err) {
      console.error("Failed to load bookmarks or groups:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Extract unique groups combined with explicitly created dbGroups
  const mergedGroups = useMemo(() => {
    const map = new Map<string, number>();

    // Seed with explicitly created db groups (ensures empty groups show up)
    dbGroups.forEach((g) => {
      if (g.name !== "Unsorted") {
        map.set(g.name, 0);
      }
    ...
    // Count bookmarks for each group
    bookmarks.forEach((b) => {
  ...
  const filteredBookmarks = useMemo(() => {
    ...
    if (filterStatus === "unread") {
      ...
      result = result.filter(
        (b) => !b.isRead && b.remindAt && b.remindAt <= now,
      );
    }
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        ...
        if (meta) {
          ...
          if (meta.description && !formDescription)
  ...
  const handleToggleRead = async (b: any) => {
    try {
      const updated = await bookmarkRepository.update(b.id, {
        isRead: !b.isRead,
      });
      if (updated)
        setBookmarks(
          bookmarks.map((item) => (item.id === b.id ? updated : item)),
        );
      syncManager.sync();
      loadStats();
    } catch (err) {
  ...
  const openArchive = (b: any) => {
    ...
    setArchiveGroupInput(
      "archive-" +
        (b.group ? b.group.toLowerCase().replace(/\s+/g, "-") : "general"),
    );
  ...
  const openTickTick = async (b: any) => {
    ...
    try {
      ...
      if (status.connected) {
        ...
        setSelectedTicktickProject(
          status.defaultProjectId || projs[0]?.id || "",
        );
      }
    } catch (err) {
  ...
  const confirmPushTickTick = async () => {
    ...
    try {
      ...
      showToast(
        "Push Failed",
        "Failed to push to TickTick: " + (err.message || "Unknown error"),
        "error",
      );
    } finally {
  ...
  const getGroupColor = (name: string) => {
    ...
    return defaultGroupColors[name] || "cyan";
  };
    ...
    try {
      if (!oldName) {
        ...
        if (group && oldName !== newName) {
          ...
          // Move bookmarks offline
          const items = await db.bookmarks
            .filter((b) => b.group === oldName)
            .toArray();
          for (const item of items) {
            await bookmarkRepository.update(item.id, { group: newName });
          }
          if (activeGroup === oldName) setActiveGroup(newName);
          loadBookmarks();
        } else if (group && oldName === newName) {
          await groupRepository.update(group.id, { color: formGroupColor });
          loadBookmarks();
        }
      }
      setShowEditGroupModal(false);
  ...
  const deleteGroup = async (groupName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (
      !confirm(
        `Delete group "${groupName}"? Bookmarks inside will be moved to "Unsorted".`,
      )
    )
      return;
    ...
    if (group) {
      ...
      const items = await db.bookmarks
        ...
        .toArray();
      for (const item of items) {
        await bookmarkRepository.update(item.id, { group: "Unsorted" });
      }
  ...
  const openAddModal = () => {
    ...
    setTimeout(() => addUrlInputRef.current?.focus(), 50);
  };
  ...
  const openEdit = (b: any) => {
    ...
    const color = getGroupColor(b.group || "Unsorted");
  ...
  const getGroupColorClass = (name: string) => {
    ...
    if (color === "cyan")
      return "text-cyber-cyan border-cyber-cyan bg-cyber-cyan/5 shadow-[0_0_8px_rgba(0,240,255,0.15)]";
    if (color === "pink")
      return "text-cyber-pink border-cyber-pink bg-cyber-pink/5 shadow-[0_0_8px_rgba(255,0,127,0.15)]";
    if (color === "green")
      return "text-cyber-green border-cyber-green bg-cyber-green/5 shadow-[0_0_8px_rgba(57,255,20,0.15)]";
    if (color === "yellow")
      return "text-cyber-yellow border-cyber-yellow bg-cyber-yellow/5 shadow-[0_0_8px_rgba(255,230,0,0.15)]";

    const n = name.toLowerCase();
    if (n.includes("read") || n.includes("later"))
      return "text-cyber-cyan border-cyber-cyan bg-cyber-cyan/5 shadow-[0_0_8px_rgba(0,240,255,0.15)]";
    if (n.includes("inspire") || n.includes("design"))
      return "text-cyber-pink border-cyber-pink bg-cyber-pink/5 shadow-[0_0_8px_rgba(255,0,127,0.15)]";
    if (n.includes("resource") || n.includes("dev"))
      return "text-cyber-green border-cyber-green bg-cyber-green/5 shadow-[0_0_8px_rgba(57,255,20,0.15)]";
    return "text-cyber-yellow border-cyber-yellow bg-cyber-yellow/5 shadow-[0_0_8px_rgba(255,230,0,0.15)]";
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--color-bg-main)] text-[var(--color-text-primary)] font-sans">
        ...
        <header className="flex-none h-16 bg-[var(--color-bg-surface)] border-b border-[var(--color-border-default)] px-4 sm:px-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden p-2 text-[var(--color-text-muted)] hover:bg-[var(--color-bg-hover)] active:scale-95 rounded-md transition-all"
            >
                  ...
                  {bookmarks.length === 0 ? (
                    ...
                    <div className="max-w-2xl mx-auto mt-12 studio-card p-8 text-center space-y-6">
                      ...
                      <p className="text-[var(--color-text-muted)] text-sm max-w-md mx-auto leading-relaxed">
                        Your clean, high-performance link vault. Save links,
                        organize them in groups, and set reminders to read them
                        later.
                      </p>
                      <div className="pt-4">
                        <button
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-6 mt-6">
                      {filteredBookmarks.map((b) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="relative aspect-[1.4/1] bg-[var(--color-bg-element)] border-b border-[var(--color-border-default)] overflow-hidden">
                          ...
                          <div className="px-2.5 sm:px-4 py-2 sm:py-2.5 border-t border-[var(--color-border-default)] flex items-center justify-between bg-[var(--color-bg-surface)] gap-1">
                            ...
                            <div className="flex items-center gap-1 sm:gap-1.5 w-full sm:w-auto justify-around sm:justify-end">
                              <button
                                ...
                                className="text-[var(--color-text-muted)] hover:text-blue-500 hover:bg-blue-50 rounded p-1.5 transition-colors active:scale-95 font-bold text-xs"
      ...
      <button
        onClick={openAddModal}
        className="fixed bottom-6 right-6 z-40 btn-primary p-3.5 sm:px-5 sm:py-3 rounded-full shadow-lg hover:shadow-xl flex items-center gap-2 active:scale-95 transition-all"
        title="Add Bookmark"
      >
      ...
      {/* User Guide Modal */}
      <AnimatePresence>
        {showUserGuide && <UserGuideModal onClose={closeUserGuide} />}
      </AnimatePresence>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full studio-input px-3.5 py-2.5 text-sm pr-10"
                  required
                  autoFocus
                />
                {isScrapingMeta && (
                  <div className="absolute right-3 flex items-center pointer-events-none text-[var(--color-accent)]">
            ...
            {!ticktickConnected ? (
              <div className="space-y-4 py-2 text-center">
                <p className="text-xs text-[var(--color-text-muted)]">
                  Your TickTick account is not connected yet. Connect your
                  account in Settings to push bookmarks as tasks.
                </p>
                <button
                  onClick={() => {
                    setShowTickTickModal(false);
              ...
              <div className="space-y-4">
                <div className="bg-[var(--color-bg-element)] border border-[var(--color-border-default)] p-3 rounded-md text-xs space-y-1">
                  <span className="text-[10px] text-[var(--color-text-muted)] font-semibold block uppercase">
                    Bookmark Title
                  </span>
                  <span className="text-[var(--color-text-primary)] font-semibold block truncate">
                    {bookmarkToPush.title}
                  </span>
                </div>
      ...
      {showDeleteModal && bookmarkToDelete && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="studio-card w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-[var(--color-status-error)]">
                Confirm Delete
              </h3>
              <button
      ...
      {showEditGroupModal && (
        <div
          className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowEditGroupModal(false);
          }}
        >
          <div className="studio-card w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-[var(--color-text-primary)]">
                {editingGroupName ? "Edit Group Name" : "Create Group"}
              </h3>
              <button

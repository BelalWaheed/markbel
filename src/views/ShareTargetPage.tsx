...
export default function ShareTargetPage() {
  ...
  const [selectedGroup, setSelectedGroup] = useState('Unsorted')
  ...
  const [existingGroups, setExistingGroups] = useState<string[]>(['Unsorted'])
  ...
  // 1. Fetch current groups from Dexie
  ...
  useEffect(() => {
    async function loadGroups() {
      try {
        ...
        setExistingGroups(['Unsorted', ...groups.map(g => g.name).filter(n => n !== 'Unsorted')])
        
        // Also map colors
        const colors: Record<string, string> = {}
        groups.forEach(g => colors[g.name] = g.color)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (showNewGroupInput && newGroupInput.trim()) {
        const existingGroup = await db.groups.where('name').equals(groupToSave).first()
        if (!existingGroup) {
